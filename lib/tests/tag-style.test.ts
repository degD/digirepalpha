...
describe("tagClassName", () => {
  it("uses fixed colors for known tags", () => {
    assert.equal(tagClassName("jazz"), "bg-violet-100 text-violet-700");
    assert.equal(tagClassName("blues"), "bg-blue-100 text-blue-700");
    assert.equal(tagClassName("rock"), "bg-red-100 text-red-600");
  });
  ...
  it("assigns a stable color to other tags", () => {
    ...
    assert.equal(tagClassName("PRACTICE"), colored);
    assert.match(colored, /^bg-\w+-100 text-\w+-700$/);
  });
